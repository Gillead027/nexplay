...
| Modo de notificação e "Silenciar" da categoria | `PARTIAL` — o modo (`all`, `mentions`, `none`) é gravado por usuário em `category_prefs` e sincronizado, mas **nada no app o lê**: não há notificação para silenciar. "Config. de notificação" ainda muda de modo **em silêncio** (só `none` aparece, como ✓ em outro item) e o `PATCH` não trata erro. Correção mínima e honesta: esconder os dois itens até existir notificação |
...
| "Marcar como lida" da categoria | `BROKEN` — função vazia, depende do rastreio de leitura (ver Roteiro 3 e Roteiro 14) |
