...
| Sessão expirada com o app aberto | `BROKEN` — o WebSocket de tempo real recebe `401` no handshake, mas o cliente não inspeciona o motivo do fechamento e reconecta para sempre (15 s), sem avisar e sem voltar à tela de entrada. Detalhe em `DISCORD_USER_JOURNEYS.md` J8 e no Atlas (REALTIME_RECONNECT_SESSION_EXPIRED). Não há aviso de expiração nem timer de `expiresAt` no cliente. |
