...
**Documentos da auditoria (roteiros 0 a 13, 191 interações):** `DISCORD_UI_ATLAS.md` (36 campos por interação), `DISCORD_INTERACTION_MATRIX.md` (tabelas de 15 colunas), `DISCORD_NAVIGATION_TREE.md` (árvore de navegação) e `DISCORD_USER_JOURNEYS.md` (nove jornadas de ponta a ponta). Este plano é o resumo executivo; o detalhe e a evidência de cada linha estão nesses quatro.
...
| Duplo clique na barra de título pra maximizar | `MISSING` | A região arrastável não tem listener de `dblclick`. |
...
| Item | Status |
|---|---|
| Layout 3 colunas (rail/canais/conteúdo/membros) | `PARTIAL` — rail, canais e conteúdo `DONE`. Corrigido na auditoria do Roteiro 13: a 4ª coluna ("MEMBROS") só existe na tela de voz **com call ativa** e lista apenas quem está na call; em canal de texto não há painel de membros nenhum |
| Lista de membros do servidor (por cargo, por presença) | `MISSING` — `getMembers(serverId)` já existe na API (hoje só usado em Config. do Servidor > Membros), então uma versão sem presença não pede infraestrutura nova. Depende de cor e agrupamento de cargo para ficar completa |
| Botões "Mostrar membros" e "Mensagens fixadas" no cabeçalho da tela de voz | `BROKEN` — sem `onClick`, controles decorativos confirmados por clique (DOM idêntico antes e depois). Correção mínima: remover. Contraria a regra de nada sem função |
...
| "Exibir membros do cargo separadamente" (`hoist`) | `MISSING` o efeito — o switch salva o campo e nada agrupa; sem lista de membros do servidor não há onde aplicar |
| Linha de estado do painel do usuário | `BROKEN` — mostra o estado da chamada de voz, então lê "Desconectado" com o app aberto e funcionando. O Discord mostra a presença. O avatar e o nome também não são clicáveis (sem menu de status nem atalho ao próprio perfil) |
...
| Volume da transmissão persistir entre reconexões | `PARTIAL` — reseta para 100 % ao reconectar (Atlas, SCREEN_SHARE_VOLUME) |
...
| Mini-perfil / popover | `PARTIAL` — abre de todos os pontos esperados, mostra atividade rica e tem ações de amizade e bloqueio que trocam sozinhas pelo tempo real. Defeitos medidos no Roteiro 13: (1) usa altura estimada de 260 px e a real é 330 px, então **vaza 30 px pela borda inferior** da janela em certas alturas; (2) o cache de perfil nunca invalida (avatar, status e bio de outra pessoa ficam velhos até o F5) e **uma falha de rede fica gravada para sempre**, sem botão de tentar de novo; (3) o foco não entra no popover nem volta ao gatilho, e pela ordem do DOM os botões dele provavelmente não são alcançáveis por Tab; (4) "Remover amigo" age sem confirmação aqui e com confirmação na aba Amigos. Sem cargos, "membro desde", servidores em comum nem selos |
