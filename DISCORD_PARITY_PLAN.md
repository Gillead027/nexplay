...
14. Chat completo — `PARTIAL` (texto em tempo real, markdown, edição/exclusão, reações, reply, pins, busca por canal, upload de arquivo/imagem e forward já funcionam; threads/emoji picker completo ainda ausentes, nenhum bloqueado por esquema)
...
Com A–H feitos e B praticamente esgotado (só resta emoji picker completo, não bloqueado por esquema), o pedido original não tem mais nenhum item de fundação de dados grande pendente. O que resta do pedido é trabalho de feature específico: categorias de canal, threads, enquetes, presença/status, indicador de não-lida, e toda a FASE 7/8 (cosméticos/Premium/server tags/boost).
