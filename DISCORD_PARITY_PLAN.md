...
| Forward (encaminhar pra outro canal/DM) | `MISSING` — DM e múltiplos servidores já existem (ver §1), então o bloqueio de esquema caiu; falta só implementar a feature em si. |
...
14. Chat completo — `PARTIAL` (texto em tempo real, markdown, edição/exclusão, reações, reply, pins, busca por canal e upload de arquivo/imagem já funcionam; threads/forward/emoji picker completo ainda ausentes, nenhum bloqueado por esquema)
...
Com A–G feitos e B praticamente esgotado (só restam forward e emoji picker completo, nenhum mais bloqueado por esquema), o pedido original não tem mais nenhum item de fundação de dados grande pendente. O que resta do pedido é trabalho de feature específico sobre essa fundação: categorias de canal, forward de mensagem, threads, enquetes, presença/status, indicador de não-lida, e toda a FASE 7/8 (cosméticos/Premium/server tags/boost).
