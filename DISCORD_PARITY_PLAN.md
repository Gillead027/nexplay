...
| Fechamento das pendências dos Roteiros 15 a 17 (Roteiro 18) | `DONE` (commit `0cd2bdb`, em produção; desktop: commit `c3fc6cc`, desktop 0.2.12 publicado em https://github.com/Gillead027/nexplay/releases/tag/v0.2.12): mensagem que não saiu vira bolha "Não enviada" com Tentar de novo e Descartar (canal e conversa direta); esqueletos de carregamento em mensagens e membros; Configurações > Sobre com política de privacidade, termos de uso e 18 licenças de código aberto, e, no desktop 0.2.12, "Verificar atualizações" e "Abrir pasta de logs"; link de convite `/convite/CÓDIGO` (entra no servidor depois de criar a conta ou entrar) e "Copiar link"; "Baixar o app para Windows" na tela de entrada; campo do cadastro renomeado para "Código de cadastro". Faltam: protocolo `nexplay://`, ilustrações e botões nos vazios de amigos, e a revisão dos textos de privacidade e termos por quem administra |
| Protocolo `nexplay://`, log de inicialização e vazios com ação (Roteiro 19) | `DONE` (commit `db467bb`, desktop 0.2.13 publicado em https://github.com/Gillead027/nexplay/releases/tag/v0.2.13; web: commit `44c7563`, em produção): `nexplay://convite/<CÓDIGO>` abre o app desktop (ou usa o que já está aberto) e entra no servidor, com validação estrita do formato no processo principal; o navegador oferece "Abrir no aplicativo" no aviso de convite; o log de inicialização passa para a pasta que "Abrir pasta de logs" abre; conversas e amigos vazios ganham "Ver amigos" e "Adicionar amigo". Falta: deep link de canal e de mensagem, ilustração nos vazios (recusada, nada decorativo) e a revisão dos textos de privacidade e termos por quem administra |
| Tela de abertura com a logo animada (pedido do usuário) | `DONE` (commit `37c9cd2`, desktop 0.2.14 publicado em https://github.com/Gillead027/nexplay/releases/tag/v0.2.14): janela pequena com a logo do NexPlay, arcos de luz que orbitam, brilho que pulsa, reflexo e o passo atual (Iniciando, Conectando ao servidor, Abrindo) com a versão; fica no mínimo 1,8 s, entrega para a janela principal e esmaece; fecha na hora se o servidor não abrir ou o app não iniciar. Não mostra barra de progresso nem verificação de atualização |
| Cadastro aberto (criar conta sem código; pedido do usuário) | `DONE` (commit `6ef7387`, em produção): `OPEN_REGISTRATION=true` no `.env` da VPS. A aba "Criar conta" não pede o código; a conta nova não entra em servidor nenhum e só vê o que criar ou aquilo em que entrar por convite. O padrão do código continua fechado (`OPEN_REGISTRATION` ausente ou `false`). Limite de 10 contas por hora por endereço. Riscos que passam a existir: qualquer pessoa pode criar servidores, canais de voz e chamar o bot de música (tráfego do proxy pago do YouTube), enviar anexos (até 15 MB) e usar banda da voz; não há limite de servidores por conta nem cota por conta |
