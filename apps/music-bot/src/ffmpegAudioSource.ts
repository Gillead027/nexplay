...
export const PCM_FRAME_BYTES = TEST_AUDIO_SAMPLES_PER_CHANNEL * TEST_AUDIO_CHANNELS * 2;
...
export class PcmFrameBuffer {
  ...
  push(chunk: Uint8Array): Int16Array[] {
    ...
    while (this.remainder.length >= PCM_FRAME_BYTES) {
      ...
      const samples = new Int16Array(TEST_AUDIO_SAMPLES_PER_CHANNEL * TEST_AUDIO_CHANNELS);
...
function createWavHeader(dataBytes: number): Buffer {
  ...
  header.writeUInt32LE(TEST_AUDIO_SAMPLE_RATE * TEST_AUDIO_CHANNELS * 2, 28);
  header.writeUInt16LE(TEST_AUDIO_CHANNELS * 2, 32);
...
export function ensureDiagnosticAudioFile(): string {
  const path = join(tmpdir(), 'sausimusic-ffmpeg-diagnostic-stereo-v2.wav');
  if (existsSync(path)) return path;

  const totalSamples = Math.floor(TEST_AUDIO_SAMPLE_RATE * (DIAGNOSTIC_AUDIO_DURATION_MS / 1_000));
  const pcm = Buffer.alloc(totalSamples * TEST_AUDIO_CHANNELS * 2);
  for (let index = 0; index < totalSamples; index += 1) {
    ...
    const sample =
      ...
      7_000 *
      Math.max(0, envelope);
    pcm.writeInt16LE(Math.max(-32_768, Math.min(32_767, Math.round(sample))), index * 4);
    pcm.writeInt16LE(Math.round(Math.sin(2 * Math.PI * 523.25 * time) * 7_000 * Math.max(0, envelope)), index * 4 + 2);
  }
