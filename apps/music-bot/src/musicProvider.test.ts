...
describe('fontes permitidas', () => {
  it('não troca YouTube por SoundCloud quando a busca falha', async () => {
    const youtube = new YouTubeProvider({ metadata: async () => { throw new Error('YouTube exige autenticação'); }, playlistMetadata: async () => [] });
    const registry = new MusicProviderRegistry([youtube], 'youtube');
    await assert.rejects(registry.resolveInput('Matuê'), /autenticação/);
    await assert.rejects(registry.resolveInput('https://soundcloud.com/artist/track'), /Nenhum provider suporta/);
  });
});
