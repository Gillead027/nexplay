// Formato e animação de imagens, pelos primeiros bytes. Serve ao app (para guardar o ícone/painel do servidor como
// veio, sem recomprimir nem perder a animação) e à API (para conferir o que chegou e saber se é animado).

export type ImageFormat = 'gif' | 'png' | 'webp' | 'jpeg';
...
const DATA_URL_PREFIX = /^data:image\/(gif|png|webp|jpeg);base64,/;

export interface ParsedImageDataUrl {
  // Formato declarado na data: URL.
  declared: ImageFormat;
  bytes: Uint8Array;
  // Formato de verdade, pelos bytes (null se não for uma imagem reconhecida).
  actual: ImageFormat | null;
  animated: boolean;
}

// Decodifica uma data: URL de imagem. Devolve null se não tiver o formato esperado.
export function parseImageDataUrl(dataUrl: string): ParsedImageDataUrl | null {
  ...
  try {
    ...
    return { declared: match[1] as ImageFormat, bytes, actual, animated: actual ? isAnimatedImage(bytes, actual) : false };
  } catch {
    return null;
  }
}
