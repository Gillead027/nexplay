...
- `scripts/watch-site.ps1`: roda neste computador (fora da VPS) e avisa por notificação nativa do Windows quando `/api/health` para de responder ou volta ao normal — cobre o caso em que a VPS inteira cai, sem depender de nenhum serviço externo. Não fica registrado como tarefa agendada (uma tarefa a cada poucos minutos fazia a janela do PowerShell piscar durante os jogos); rode à mão quando quiser conferir.
- Novidades (`packages/shared/src/changelog.ts`): toda atualização que chega aos usuários ganha uma entrada ali, escrita em português. Na próxima subida da API (o deploy), cada entrada nova vira uma mensagem do NexPlay no canal **atualizações** de todos os servidores. Esse canal é criado sozinho: os servidores que já existiam ganham o seu uma única vez, e todo servidor novo já nasce com ele (só quem gerencia mensagens escreve ali; se o dono apagar o canal, ele não volta). Uma entrada publicada nunca deve ter o `id` mudado, senão sai de novo.
...
- **Quem está assistindo a transmissão**: quem transmite vê quantas pessoas estão assistindo (na barra de controles, com um olhinho, e no quadrado da transmissão); quem ainda não abriu vê "N assistindo"; dentro da transmissão aberta também aparece a contagem. Um olhinho fica ao lado do nome de quem está assistindo alguma transmissão (na lista de canais e no quadrado da pessoa), como o ícone de microfone mutado. Quando alguém abre ou fecha uma transmissão que é sua (ou uma que você assiste) toca um toquinho agudo de entrada ou de saída, diferente do som de entrar e sair da chamada; quem já estava assistindo antes de você chegar não faz barulho, e quem sai da chamada inteira só toca o som de sair da chamada. Cada pessoa publica o que está assistindo como atributo da chamada (`watching`), então versões antigas do app que ainda não publicam não são contadas.
- **Cronômetro da chamada**: cada canal de voz com gente mostra há quanto tempo a chamada está ativa (na lista de canais, no cabeçalho da sala e em "Voz conectada"). A chamada começa quando a primeira pessoa entra numa sala vazia e termina quando a última sai (o bot de música sozinho não conta); o começo fica guardado no banco, então reiniciar a API no meio da chamada não zera o cronômetro.
- **Excluir conta**: em Configurações > Conta e segurança ("Excluir minha conta", pede a senha) e, para o admin da instância, na tabela de Contas do painel de Administração. Mostra antes o que acontece com os servidores da conta (passam para outra pessoa ou são apagados se ninguém mais está neles). Contas de administrador da instância (ADMIN_USERNAMES) não podem ser excluídas.
- **Ligações individuais**: em Amigos, o botão de telefone ao lado de cada amigo (e o botão "Ligar" no cabeçalho da conversa) chama a pessoa. Ela recebe um aviso com toque (mudo em "Não perturbe") e escolhe Atender ou Recusar; se ninguém atender em 45 s a ligação é perdida para os dois. Depois de atendida tem as mesmas opções da chamada de um servidor (microfone, fone, câmera, compartilhar tela, cronômetro, mutar/ensurdecer visíveis para o outro), com a conversa por texto logo abaixo. Só amigos que não se bloquearam ligam, e o outro precisa estar com o app aberto. Sair da ligação não derruba o outro (dá para voltar por "Entrar na ligação"); ela acaba quando os dois saem. Cada ligação é uma sala própria no LiveKit ("dm-" + id da conversa) e o token só sai para os dois participantes.
- **Diagnóstico da chamada**: Configurações > Voz e vídeo mostra ao vivo perda de pacotes, oscilação, atraso, falhas na voz, quadros descartados e se o computador ou a internet estão limitando a câmera, com frases dizendo de quem é o problema, e um botão para copiar o relatório.
