...
# O music-bot fica de fora de propósito (é isolado; ver README). Da pasta infra/ só vai o nginx.conf (o do site): o resto tem,
# na VPS, as chaves de produção (livekit.local.yaml) e o domínio, que não podem ser trocados pelos de desenvolvimento.
set -euo pipefail
...
tar --exclude=node_modules --exclude=dist --exclude=data --exclude=secrets --exclude='*.db' --exclude='*.db-*' \
    ...
    -czf "$TARBALL" apps packages scripts infra/nginx.conf package.json package-lock.json tsconfig.base.json README.md
