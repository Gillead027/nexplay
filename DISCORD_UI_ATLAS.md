...
**MENU**: Não aplicável.
**MODAL**: Não aplicável.
**SEGUNDA ETAPA**: Backend, na ordem exata do código: (1) valida `registerSchema`; (2) checa `inviteMatches(inviteToken)`; (3) normaliza espaços do username (`replace(/\s+/g, ' ')`); (4) checa duplicidade (`getUserByUsername`); (5) `createUser` (hash de senha); (6) **CORRIGIDO (commit `4e9708c`, em produção)**: a conta nova já não entra em servidor nenhum sozinha (antes `addServerMember` + `assignDefaultRole` no servidor mais antigo, `getDefaultServerId()`); sem servidor, a tela mostra "Você ainda não está em nenhum servidor" com "Entrar com convite" e "Criar servidor"; (7) `createSession` + `setSessionCookie`; (8) `201` com `{ user }`.
